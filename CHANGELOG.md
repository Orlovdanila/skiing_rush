...
### Boundary Cooldown + Динамические границы

**Проблема:** При столкновении с краем и удержании клавиши персонаж колебался с затуханием и "прилипал" к границе. Также границы были захардкожены (margin = 50px).

**Решение:**

1. **gameConfig.ts** - новые параметры:
   ```typescript
   PLAYER_PHYSICS.boundaryCooldownTime: 250  // ms блокировки ввода к границе
   
   BOUNDARY_CONFIG = {
     fenceWidth: 60,      // Ширина ассета забора (при baseWidth 1920)
     playerMargin: 20,    // Доп. отступ от забора
     baseWidth: 1920      // Референсная ширина для масштабирования
   }
   ```

2. **Player.ts**:
   - Добавлены `boundaryCooldown` и `lastBoundaryHitDirection`
   - При столкновении: cooldown 250ms + запоминаем сторону
   - Во время cooldown: ввод в сторону границы игнорируется (но в противоположную - работает)
   - Margin теперь динамический: `(fenceWidth + playerMargin) * (screenWidth / 1920)`

**Результат:** Зигзагообразная траектория при удержании клавиши у края (как на скриншоте 5.png).
